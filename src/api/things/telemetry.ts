...
export type TsKvEntity = Record<string, Array<TsData>>;
...
export function getLatestTimeseries(entityId: EntityId<any>, keys?: string, useStrictDataTypes?: boolean) { //useStrictDataTypes  数值转换为字符串
...
export function saveEntityAttributesV1(entityId: EntityId<any>, scope: Scope.SERVER_SCOPE | Scope.SHARED_SCOPE, data: any) {
  return defHttp.postJson<void>({
    url: `/api/plugins/telemetry/${entityId.entityType}/${entityId.id}/${scope}`,
    data
  });
}
export function saveEntityAttributesV2(entityId: EntityId<any>, scope: Scope.SERVER_SCOPE | Scope.SHARED_SCOPE, data: any) {
  return defHttp.postJson<void>({
