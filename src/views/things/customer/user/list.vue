<template>
  <div class="customer-user-list">
    <BasicTable @register="registerTable">
      <template #headerTop>
        {{ t(getTitle.value) }} ( {{ customer.title }})
      </template>
      <template #leftToolbar>
        <a-button type="primary" @click="handleForm({})">
          <Icon icon="fluent:add-12-filled" /> 新增用户
        </a-button>
...
import { getCustomerById, Customer } from '/@/api/things/customer';
import { getCustomerUsers, deleteUser, getUserToken, getUserById } from '/@/api/things/user';
...
function handleForm(record: Recordable) {
  openModal(true, { ...record, authority: Authority.CUSTOMER_USER, customerId: customer.value.id, tenantId: customer.value.tenantId });
}
