<template>
  <div class="device-profile-alarm-form">
    <CollapseContainer v-for="(alarm, index) in alarmList" :key="index" class="border border-neutral-300 mb-2 ">
      <template #title>
        {{ alarm.alarmType }}
      </template>
      <template #action>
      ...
      <div v-if="alarm.propagate == true" class="mb-4">
        <div class="flex items-center">
          ...
          <Select class="flex-1" :allowClear="true" placeholder="要传递的关联类型" mode="tags" :open="false"
      ...
      <div class="mb-4 mt-2">
        ...
        <div v-else>
          <div v-for="(item, createRuleIndex) in alarm.createRulesList" :key="createRuleIndex" class="flex items-center">
            <div class="md:w-13/14 border border-neutral-400 rounded p-2 mb-2 ">
              <div class="md:flex">
                <div class="md:w-1/4 w-full border-r-1 border-slate-300 pr-1">
                  <div>报警等级</div>
                  <Select class="w-full" placeholder="报警等级" v-model:value="item.severity"
                ...
                <div class="md:w-3/4 w-full pl-2">
                  ...
                  <div class="flex items-center">
                    ...
                    <Select class="flex-1 w-full" :allowClear="true" v-model:value="item.rule.dashboardId.id"
            ...
            <div class="md:w-1/14">
              <Tooltip :title="'移除'" class="mx-2">
                <Icon class="cursor-pointer" :icon="'ant-design:minus-circle-outlined'" color="red" :size="20"
                  @click="handleDeleteCreateRule(index, createRuleIndex)" />
              </Tooltip>
            </div>
          </div>
        </div>
        <a-button v-if="(alarm.createRulesList?.length || 0) < 6" type="default" size="small"
      ...
      <div class="mb-4 mt-2">
        <div v-if="alarm.clearRule != undefined" class="flex items-center">
          <div class="md:w-13/14 border border-neutral-400 rounded p-2 mb-2 ">
            ...
            <div class="flex items-center">
              <label class="float-right mr-2">移动仪表盘：</label>
              <Select class="flex-1 w-full" :allowClear="true" v-model:value="alarm.clearRule.dashboardId.id"
                :options="dashboardList"></Select>
            </div>
          </div>
          <div class="md:w-1/14">
            <Tooltip :title="'移除'" class="mx-2">
...
import { Alarm, AlarmRule } from '/@/api/things/deviceProfile';
...
async function validateFieldsValue(nameList?: NamePath[]) {
  ...
  alarmList.value.forEach(item => {
    ...
    const clearRule = isEmpty(item.clearRule) ? undefined : { ...item.clearRule, dashboardId: isEmpty(item.clearRule.dashboardId?.id) ? undefined : item.clearRule.dashboardId };
    const createRules = {};
    if (item.createRulesList) {
      ...
      item.createRulesList.forEach(createRuleItem => {
        ...
        createRules[createRuleItem.severity] = { ...createRuleItem.rule, dashboardId: isEmpty(createRuleItem.rule.dashboardId?.id) ? undefined : createRuleItem.rule.dashboardId };
      })
    }
    data.push({
      id: item.id,
...
function handleAddCreateRule(index: number) {
  ...
  createRulesList.push({ severity: AlarmSeverity.INDETERMINATE, rule: { schedule: { type: 'ANY_TIME' }, dashboardId: { entityType: EntityType.DASHBOARD, id: '' } } as AlarmRule });
...
function handleAddClearRule(index: number) {
  alarmList.value[index].clearRule = { schedule: { type: 'ANY_TIME' }, dashboardId: { entityType: EntityType.DASHBOARD, id: '' } } as AlarmRule;
}
