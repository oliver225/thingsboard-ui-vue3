    ...
    <BasicTable @register="registerTable">
      ...
      <template #leftToolbar>
        <a-button type="primary" @click="handleForm({})" v-show="userStore.getAuthority == Authority.TENANT_ADMIN">
          <Icon icon="fluent:add-12-filled" /> 新增实体视图
        </a-button>
...
import { getEntityViewTypes, deleteEntityView, getTenantEntityViewInfos, getCustomerEntityViewInfos, unAssignEntityViewFromCustomer, assignEntityViewToPublicCustomer } from '/@/api/things/entityView';
...
function wrapFetchParams(fetchParam: any) {
  ...
  const type = fetchParam.type ? fetchParam.type[0] : null;
  return { ...fetchParam, page: page, textSearch: searchParam.textSearch, type: type }
}
  ...
  return await getEntityViewTypes().then((result) => {
    ...
    return (
      userStore.getAuthority == Authority.CUSTOMER_USER
        ? getCustomerEntityViewInfos(param, userStore.userInfo?.customerId.id || '',)
