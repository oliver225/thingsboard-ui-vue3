...
import { EntityRelation, saveRelation } from '/@/api/things/relation';
...
import { getTenantEntityViewInfos } from '/@/api/things/entityView';
import { tenantById } from '/@/api/things/tenant';
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  await onEntityTypeChange(direction.value == 'From' ? record.value.to?.entityType : record.value.from?.entityType, direction.value == 'From' ? 'To' : 'From')
...
async function onEntityTypeChange(entityType: EntityType, direction: 'From' | 'To') {
  ...
  switch (entityType) {
    case EntityType.DEVICE:
      const deviceListResult = await getTenantDeviceInfoList({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' });
      options = deviceListResult.data.map(device => ({ label: device.name, value: device.id.id }))
      break;
    case EntityType.ASSET:
      const assetListResult = await getTenantAssetInfoList({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' });
      options = assetListResult.data.map(device => ({ label: device.name, value: device.id.id }))
      break;
    case EntityType.ENTITY_VIEW:
      const entityViewListResult = await getTenantEntityViewInfos({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' });
    ...
    case EntityType.CUSTOMER:
      const customerListResult = await customerList({ pageSize: 50, page: 0, sortProperty: 'title', sortOrder: 'ASC' });
    ...
    case EntityType.DASHBOARD:
      const dashboardListResult = await currentTenantDashboardList({ pageSize: 50, page: 0, sortProperty: 'title', sortOrder: 'ASC' });
      options = dashboardListResult.data.map(device => ({ label: device.name, value: device.id.id }))
      break;
    default:
      options = [];
  }
