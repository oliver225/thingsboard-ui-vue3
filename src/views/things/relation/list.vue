    ...
    <BasicTable @register="registerTable">
      ...
      <template #toolbar>
        <Tooltip placement="bottom">
          ...
          <Icon icon="ant-design:plus-outlined" :size="20" class="cursor-pointer" @click="handleForm({})"
            v-if="userStore.getAuthority == Authority.TENANT_ADMIN" />
        </Tooltip>
      </template>
      <template #leftToolbar>
        <Segmented v-model:value="direction" @change="handleDirectionChange"
          :options="[{ label: '向外(From)', value: 'From' }, { label: '向内(To)', value: 'To' }]" />
      ...
      <template #extra="{ record }">
        <span v-if="direction == 'To'">
          从
          <strong>
            {{ ENTITY_TYPE_OPTIONS.find((item) => item.value === record.from.entityType)?.label }} ({{ record.fromName }})
          </strong>
...
import { findRelationInfoListByFrom, findRelationInfoListByTo, deleteRelation } from '/@/api/things/relation';
        ...
        await deleteRelation(
          {
            ...
            fromType: record.from.type,
            toId: record.to.id,
            toType: record.to.type,
