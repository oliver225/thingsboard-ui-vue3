...
import { queueList, deleteQueue, Queue } from '/@/api/things/queue';
...
const tableColumns: BasicColumn[] = [
  ...
  },
  {
    ...
    sorter: true,
    align: 'left',
    format: (text: any) => text.type ? SUBMIT_STRATEGY_OPTIONS.find((item) => item.value === text.type)?.label || text.type : ''
  },
  {
    title: '处理策略',
    ...
    sorter: true,
    align: 'left',
    format: (text: any) => text.type ? PROCESSING_STRATEGY_OPTIONS.find((item) => item.value === text.type)?.label || text.type : ''
  },
