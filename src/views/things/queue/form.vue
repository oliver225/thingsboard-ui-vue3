  ...
  <BasicModal v-bind="$attrs" :showFooter="true" @register="registerModal" @ok="handleSubmit" width="50%">
    ...
    <Form ref="formRef" :model="formState" layout="vertical">
      ...
      <CollapseContainer title="提交设置" class="border border-neutral-300 mb-4">
        <Row :gutter="16">
          <Col :span="12">
          <Form.Item label="提交策略" :name="['submitStrategy', 'type']" :rules="[{ required: true, message: '请选择提交策略!' }]">
          ...
          <Col :span="12">
          <Form.Item label="批量处理大小" :name="['submitStrategy', 'batchSize']"
            ...
            <InputNumber :style="{ width: '90%' }" :defaultValue="1000" :min="1"
              v-model:value="formState.submitStrategy.batchSize" placeholder="请输入批量处理大小" />
          </Form.Item>
          </Col>
        </Row>
      </CollapseContainer>
      <CollapseContainer title="重试处理设置" class="border border-neutral-300 mb-4">
        <Row :gutter="16">
          <Col :span="12">
          <Form.Item label="处理策略" :name="['processingStrategy', 'type']"
          ...
          <Col :span="12">
          <Form.Item label="重试次数(0表示无限制)" :name="['processingStrategy', 'retries']"
            :rules="[{ required: true, message: '请输入重试次数!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
              v-model:value="formState.processingStrategy.retries" placeholder="请输入重试次数" />
          </Form.Item>
          <Form.Item label="跳过重试的失败消息百分比" :name="['processingStrategy', 'failurePercentage']"
            :rules="[{ required: true, message: '请输入跳过重试的失败消息百分比!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="0" :min="0" :max="100"
              v-model:value="formState.processingStrategy.failurePercentage" placeholder="请输入跳过重试的失败消息百分比" />
          </Form.Item>
          <Form.Item label="重试间隔" :name="['processingStrategy', 'pauseBetweenRetries']"
            :rules="[{ required: true, message: '请输入重试间隔!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
              v-model:value="formState.processingStrategy.pauseBetweenRetries" placeholder="请输入重试间隔" addon-after="秒" />
          </Form.Item>
          <Form.Item label="最大重试间隔" :name="['processingStrategy', 'maxPauseBetweenRetries']"
            :rules="[{ required: true, message: '请输入最大重试间隔!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
              v-model:value="formState.processingStrategy.maxPauseBetweenRetries" placeholder="请输入最大重试间隔"
              addon-after="秒" />
          </Form.Item>
          </Col>
        </Row>
      ...
      <CollapseContainer title="轮训设置" class="border border-neutral-300 mb-4">
        ...
        <Row :gutter="16">
          ...
          <Form.Item label="轮训间隔" name="pollInterval" :rules="[{ required: true, message: '请输入轮训间隔!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="25" :min="0" v-model:value="formState.pollInterval"
              placeholder="请输入轮训间隔" />
          </Form.Item>
          </Col>
          <Col :span="12">
          <Form.Item label="分区" name="partitions" :rules="[{ required: true, message: '请输入分区!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="10" :min="1" v-model:value="formState.partitions"
              placeholder="请输入分区" />
          </Form.Item>
          </Col>
        </Row>
        ...
        <Row :gutter="16">
          ...
          <Form.Item label=" " name="consumerPerPartition">
            <Checkbox v-model:checked="formState.consumerPerPartition" :defaultChecked="false"> 每个分区消费者单独轮询消息</Checkbox>
          </Form.Item>
          </Col>
          <Col :span="12">
          <Form.Item label="处理超时" name="packProcessingTimeout" :rules="[{ required: true, message: '请输入处理超时时间!' }]">
            <InputNumber :style="{ width: '90%' }" :defaultValue="2000" :min="0"
              v-model:value="formState.packProcessingTimeout" placeholder="请输入处理超时时间" addon-after="毫秒" />
          </Form.Item>
          </Col>
        </Row>
      </CollapseContainer>
      <Form.Item label="描述信息" :name="['additionalInfo', 'description']" help="此文本将显示在队列说明中，而不是所选策略中">
        <Textarea v-model:value="formState.additionalInfo.description" placeholder="输入队列描述信息" :rows="3" />
      </Form.Item>
    </Form>
  </BasicModal>
</template>
<script lang="ts" setup name="QueueForm">
...
import { saveQueue, getQueueById, Queue } from '/@/api/things/queue';
