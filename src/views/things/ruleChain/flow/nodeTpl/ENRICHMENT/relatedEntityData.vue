    ...
    <Form ref="formRef" :model="formState" layout="vertical">
        <div class="border border-neutral-300 rounded-md py-2 px-3 mb-4">
            ...
            <div class="p-2">
                <Row :gutter="20">
                    <Col :span="12">
                    <Form.Item label="方向" :name="['relationsQuery', 'direction']"
                    ...
                    <Form.Item label="最大关联层级" :name="['relationsQuery', 'maxLevel']">
                        <InputNumber v-model:value="formState.relationsQuery.maxLevel" :precision="0" :step="1" :min="1"
                            :style="{ width: '100%' }" />
                    </Form.Item>
                    </Col>
                </Row>
                <Form.Item :name="['relationsQuery', 'fetchLastLevelOnly']"
                ...
                <div class="border border-neutral-300 rounded-md py-2 px-3 mb-4">
                    <Form.Item label="关联筛选器" :name="['relationsQuery', 'filters']"
                        ...
                        <div class="p-2">
                            <Table class="mapping-table">
                                ...
                                <tr class="content" v-for="(filter, index) in formState.relationsQuery.filters"
                                    :key="index">
                                    <td class="py-2 px-4">
                                        <Select v-model:value="filter.relationType" :style="{ width: '100%' }"
                                    ...
                                    <td>
                                        <Select v-model:value="filter.entityTypes" mode="multiple"
                                            :style="{ width: '100%' }" :options="entityTypeOptions">
                                        </Select>
                                    </td>
                                    <td>
                                        <Tooltip title="删除" class="pl-4">
                                            <Icon :icon="'ant-design:delete-outlined'" :size="20" color="red"
                                                class="cursor-pointer" @click="handleDeleteFilter(index)" />
                                        </Tooltip>
                                    </td>
                                </tr>
                            </Table>
                        ...
                        <Table class="mapping-table">
                            ...
                            <tr class="content" v-for="(mapping, index) in mappingList" :key="index">
                                ...
                                <td>
                                    <Tooltip title="删除" class="pl-4">
                                        <Icon :icon="'ant-design:delete-outlined'" :size="20" color="red"
                                            class="cursor-pointer" @click="handleDeleteMapping(index)" />
                                    </Tooltip>
                                </td>
                            </tr>
                        </Table>
                        <Button class="my-4" type="primary" @click="handleAddMapping">添加映射</Button>
                        <Alert type="success" message="只有目标键字段支持模板化。使用$[messageKey]从消息中提取值，使用${metadataKey}从元数据中提取值。" />
                    </Form.Item>
                </div>
...
import { Form, Input, Radio, Button, Tooltip, Divider, Space, Row, Col, Checkbox, InputNumber, Select, Alert } from 'ant-design-vue';
