<template>
  <BasicModal v-bind="$attrs" :showFooter="true" @register="registerModal" @ok="handleSubmit" width="50%">
    ...
    <Form ref="formRef" :model="formState" layout="vertical">
      ...
      <CollapseContainer :title="`队列(${formState.profileData.queueConfiguration?.length})`"
        v-if="formState.isolatedTbRuleEngine == true" class="border border-neutral-300 mb-4">
        <CollapseContainer v-for="(queue, index) in formState.profileData.queueConfiguration" :key="index"
          :title="queue.name" class="border border-neutral-300 mb-4">
          <template #action>
          ...
          <Form.Item label="队列名称" :name="['profileData', 'queueConfiguration', index, 'name']" :rules="[
            { required: true, message: '请输入队列名称!' },
            { pattern: /^[a-zA-Z0-9_-]*$/, message: '队列名称只能包含字母数字和 _、-', trigger: ['change', 'blur'] }]">
            <Input v-model:value="formState.profileData.queueConfiguration[index].name" :disabled="index == 0"
              placeholder="请输入队列名称"
              @change="() => formState.profileData.queueConfiguration[index].topic = `tb_rule_engine.${formState.profileData.queueConfiguration[index].name}`" />
          </Form.Item>
          ...
          <CollapseContainer title="提交设置" class="border border-neutral-300 mb-4">
            <Row :gutter="16">
              <Col :span="12">
              <Form.Item label="提交策略" :name="['profileData', 'queueConfiguration', index, 'submitStrategy', 'type']"
                ...
                <Radio.Group v-model:value="formState.profileData.queueConfiguration[index].submitStrategy.type">
                  <Radio :style="{ display: 'flex', height: '30px', lineHeight: '30px', marginLeft: '30px' }"
                    v-for="(option, index) in SUBMIT_STRATEGY_OPTIONS" :key="index" :value="option.value">
                    {{ option.label }}
              ...
              <Form.Item label="批量处理大小"
                ...
                <InputNumber :style="{ width: '90%' }" :defaultValue="1000" :min="1"
          ...
          <CollapseContainer title="重试处理设置" class="border border-neutral-300 mb-4">
            <Row :gutter="16">
              <Col :span="12">
              <Form.Item label="处理策略" :name="['profileData', 'queueConfiguration', index, 'processingStrategy', 'type']"
                ...
                <Radio.Group v-model:value="formState.profileData.queueConfiguration[index].processingStrategy.type">
                  <Radio :style="{ display: 'flex', height: '40px', lineHeight: '40px', marginLeft: '30px' }"
                    v-for="(option, index) in PROCESSING_STRATEGY_OPTIONS" :key="index" :value="option.value">
                    {{ option.label }}
                    <Tooltip placement="top" :title="option.help">
                      <Icon :icon="'ant-design:info-circle-outlined'" />
                    </Tooltip>
              ...
              <Form.Item label="重试次数(0表示无限制)"
                ...
                <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
              ...
              <Form.Item label="跳过重试的失败消息百分比"
                ...
                <InputNumber :style="{ width: '90%' }" :defaultValue="0" :min="0" :max="100"
              ...
              <Form.Item label="重试间隔"
                ...
                <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
                  v-model:value="formState.profileData.queueConfiguration[index].processingStrategy.pauseBetweenRetries"
                  placeholder="请输入重试间隔" addon-after="秒" />
              </Form.Item>
              <Form.Item label="最大重试间隔"
                ...
                <InputNumber :style="{ width: '90%' }" :defaultValue="3" :min="0"
                  v-model:value="formState.profileData.queueConfiguration[index].processingStrategy.maxPauseBetweenRetries"
                  placeholder="请输入最大重试间隔" addon-after="秒" />
              </Form.Item>
              </Col>
            </Row>
          </CollapseContainer>
          <CollapseContainer title="轮训设置" class="border border-neutral-300 mb-4">
            批量处理
            <Row :gutter="16">
              <Col :span="12">
              <Form.Item label="轮训间隔" :name="['profileData', 'queueConfiguration', index, 'pollInterval']"
                :rules="[{ required: true, message: '请输入轮训间隔!' }]">
                <InputNumber :style="{ width: '90%' }" :defaultValue="25" :min="0"
                  v-model:value="formState.profileData.queueConfiguration[index].pollInterval" placeholder="请输入轮训间隔" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="分区" :name="['profileData', 'queueConfiguration', index, 'partitions']"
                :rules="[{ required: true, message: '请输入分区!' }]">
                <InputNumber :style="{ width: '90%' }" :defaultValue="10" :min="1"
                  v-model:value="formState.profileData.queueConfiguration[index].partitions" placeholder="请输入分区" />
              </Form.Item>
              </Col>
            </Row>
            即时处理
            <Row :gutter="16">
              ...
              <Form.Item label=" " :name="['profileData', 'queueConfiguration', index, 'consumerPerPartition']">
                <Checkbox v-model:checked="formState.profileData.queueConfiguration[index].consumerPerPartition"
                  :defaultChecked="false"> 每个分区消费者单独轮询消息
                </Checkbox>
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="处理超时" :name="['profileData', 'queueConfiguration', index, 'packProcessingTimeout']"
                :rules="[{ required: true, message: '请输入处理超时时间!' }]">
                <InputNumber :style="{ width: '90%' }" :defaultValue="2000" :min="0"
                  v-model:value="formState.profileData.queueConfiguration[index].packProcessingTimeout"
                  placeholder="请输入处理超时时间" addon-after="毫秒" />
              </Form.Item>
              </Col>
            </Row>
          </CollapseContainer>
          <Form.Item label="描述信息" :name="['profileData', 'queueConfiguration', index, 'additionalInfo', 'description']"
            help="此文本将显示在队列说明中，而不是所选策略中">
            <Textarea v-model:value="formState.profileData.queueConfiguration[index].additionalInfo.description"
              placeholder="输入队列描述信息" :rows="3" />
          </Form.Item>
        </CollapseContainer>
      ...
      <CollapseContainer title="配置设置" class="border border-neutral-300 mb-4">
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="最大设备数" :name="['profileData', 'configuration', 'maxDevices']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxDevices" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            ...
            <Form.Item label="最大客户数" :name="['profileData', 'configuration', 'maxCustomers']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxCustomers" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            ...
            <Form.Item label="最大仪表盘数" :name="['profileData', 'configuration', 'maxDashboards']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxDashboards" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="最大规则链数" :name="['profileData', 'configuration', 'maxRuleChains']"
              :rules="[{ required: true, message: '请输入最大规则链数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxRuleChains" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        </Card>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="最大规则引擎执行数" :name="['profileData', 'configuration', 'maxREExecutions']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxREExecutions"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="最大传输消息数" :name="['profileData', 'configuration', 'maxTransportMessages']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxTransportMessages"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="最大 JavaScript 执行数" :name="['profileData', 'configuration', 'maxJSExecutions']"
              :rules="[{ required: true, message: '请输入最大 JavaScript 执行数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxJSExecutions"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="最大传输数据点数" :name="['profileData', 'configuration', 'maxTransportDataPoints']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxTransportDataPoints"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="每条消息的最大规则节点执行数" :name="['profileData', 'configuration', 'maxRuleNodeExecutionsPerMessage']"
              :rules="[{ required: true, message: '请输入每条消息的最大规则节点执行数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxRuleNodeExecutionsPerMessage"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="最大存储点天" :name="['profileData', 'configuration', 'maxDPStorageDays']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxDPStorageDays"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="告警TTL天数" :name="['profileData', 'configuration', 'alarmsTtlDays']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.alarmsTtlDays" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="默认存储TTL天数" :name="['profileData', 'configuration', 'defaultStorageTtlDays']"
              :rules="[{ required: true, message: '请输入默认存储TTL天数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.defaultStorageTtlDays"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="RPC TTL天数" :name="['profileData', 'configuration', 'rpcTtlDays']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.rpcTtlDays" :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        </Card>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="最大创建告警数" :name="['profileData', 'configuration', 'maxCreatedAlarms']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxCreatedAlarms"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="资源文件总大小" :name="['profileData', 'configuration', 'maxResourcesInBytes']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxResourcesInBytes"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="OTA包文件总大小" :name="['profileData', 'configuration', 'maxOtaPackagesInBytes']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxOtaPackagesInBytes"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="租户最大会话数" :name="['profileData', 'configuration', 'maxWsSessionsPerTenant']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSessionsPerTenant"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="租户最大订阅数" :name="['profileData', 'configuration', 'maxWsSubscriptionsPerTenant']"
              :rules="[{ required: true, message: '请输入租户最大订阅数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSubscriptionsPerTenant"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="客户最大会话数" :name="['profileData', 'configuration', 'maxWsSessionsPerCustomer']"
              ...
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSessionsPerCustomer"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="客户最大订阅数" :name="['profileData', 'configuration', 'maxWsSubscriptionsPerCustomer']"
              :rules="[{ required: true, message: '请输入客户最大订阅数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSubscriptionsPerCustomer"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="公共用户最大会话数" :name="['profileData', 'configuration', 'maxWsSessionsPerPublicUser']"
              :rules="[{ required: true, message: '请输入公共用户最大会话数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSessionsPerPublicUser"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="公共用户最大订阅数" :name="['profileData', 'configuration', 'maxWsSubscriptionsPerPublicUser']"
              :rules="[{ required: true, message: '请输入公共用户最大订阅数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSubscriptionsPerPublicUser"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="普通用户最大会话数" :name="['profileData', 'configuration', 'maxWsSessionsPerRegularUser']"
              :rules="[{ required: true, message: '请输入普通用户最大会话数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSessionsPerRegularUser"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="普通用户最大订阅数" :name="['profileData', 'configuration', 'maxWsSubscriptionsPerRegularUser']"
              :rules="[{ required: true, message: '请输入普通用户最大订阅数!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.maxWsSubscriptionsPerRegularUser"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="会话最大消息队列大小" help="队列的大小也受到系统配置的限制"
              :name="['profileData', 'configuration', 'wsMsgQueueLimitPerSession']"
              :rules="[{ required: true, message: '请输入会话最大消息队列大小!' }]">
              <InputNumber v-model:value="formState.profileData.configuration.wsMsgQueueLimitPerSession"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
        ...
        <Card size="small">
          ...
          <Row :gutter="20">
            ...
            <Form.Item label="传输租户消息" :name="['profileData', 'configuration', 'transportTenantMsgRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportTenantMsgRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="传输设备消息" :name="['profileData', 'configuration', 'transportDeviceMsgRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportDeviceMsgRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="传输租户遥测消息" :name="['profileData', 'configuration', 'transportTenantTelemetryMsgRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportTenantTelemetryMsgRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="传输设备遥测消息" :name="['profileData', 'configuration', 'transportDeviceTelemetryMsgRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportDeviceTelemetryMsgRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="传输租户遥测消息点数"
              :name="['profileData', 'configuration', 'transportTenantTelemetryDataPointsRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportTenantTelemetryDataPointsRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="传输设备遥测消息点数"
              :name="['profileData', 'configuration', 'transportDeviceTelemetryDataPointsRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.transportDeviceTelemetryDataPointsRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="租户REST请求" :name="['profileData', 'configuration', 'tenantServerRestLimitsConfiguration']">
              <RateLimit v-model:value="formState.profileData.configuration.tenantServerRestLimitsConfiguration"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="客户REST请求" :name="['profileData', 'configuration', 'customerServerRestLimitsConfiguration']">
              <RateLimit v-model:value="formState.profileData.configuration.customerServerRestLimitsConfiguration"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="实体版本创建" :name="['profileData', 'configuration', 'tenantEntityImportRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.tenantEntityImportRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="实体版本加载" :name="['profileData', 'configuration', 'tenantEntityExportRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.tenantEntityExportRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="WebSocket会话更新" :name="['profileData', 'configuration', 'wsUpdatesPerSessionRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.wsUpdatesPerSessionRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="Cassandra租户查询"
              :name="['profileData', 'configuration', 'cassandraQueryTenantRateLimitsConfiguration']">
              <RateLimit v-model:value="formState.profileData.configuration.cassandraQueryTenantRateLimitsConfiguration"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="通知请求" :name="['profileData', 'configuration', 'tenantNotificationRequestsRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.tenantNotificationRequestsRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
            <Col :span="12">
            <Form.Item label="每个规则通知请求"
              :name="['profileData', 'configuration', 'tenantNotificationRequestsPerRuleRateLimit']">
              <RateLimit v-model:value="formState.profileData.configuration.tenantNotificationRequestsPerRuleRateLimit"
                :style="{ width: '100%' }" />
            </Form.Item>
            </Col>
          </Row>
...
<script lang="ts" setup name="TenantProfileForm">
...
import { TenantProfile, getTenantProfileById, saveTenantProfile } from '/@/api/things/tenantProfile';
import RateLimit from './RateLimit.vue';
import { FormInstance } from 'ant-design-vue/lib/form';
import { buildUUID } from '/@/utils/uuid';
import { SUBMIT_STRATEGY_OPTIONS, PROCESSING_STRATEGY_OPTIONS, ProcessingStrategyType, SubmitStrategyType } from '/@/enums/queueEnum';
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  if (data?.id?.id) {
    ...
    if (isArray(record.value.profileData.queueConfiguration) && record.value.profileData.queueConfiguration.length > 0) {
