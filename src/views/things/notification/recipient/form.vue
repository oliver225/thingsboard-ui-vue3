...
<script lang="ts" setup name="RecipientForm">
...
import { NotificationTarget, getNotificationTargetById, saveNotificationTarget } from '/@/api/things/notificationTarget';
import { NotificationRecipientType, NotificationRecipientUsersFilter, NOTIFICATION_RECIPIENT_TYPE_OPTIONS, NOTIFICATION_RECIPIENT_USER_FILTER_OPTIONS } from '/@/enums/notificationEnum';
import { Authority } from '/@/enums/authorityEnum';
import { tenantList } from '/@/api/things/tenant';
import { tenantProfileInfoList } from '/@/api/things/tenantProfile';
import { isArray, isEmpty } from 'lodash';
import { customerList } from '/@/api/things/customer';
import { userList } from '/@/api/things/user';
...
const usersFilterOptions = computed(() => {
  if (userStore.getAuthority == Authority.SYS_ADMIN) {
    return NOTIFICATION_RECIPIENT_USER_FILTER_OPTIONS.filter((item) => {
      return item.value == NotificationRecipientUsersFilter.ALL_USERS
...
const inputFormSchemas: FormSchema[] = [
  ...
  },
  {
    ...
    componentProps: {
      ...
      mapFn: (item) => { return { label: item.email, value: item.id.id } },
      api: userList,
    },
    ifShow: false,
    colProps: { lg: 24, md: 24 },
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  if (NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == record.value?.configuration?.usersFilter?.type && userStore.getAuthority == Authority.SYS_ADMIN) {
    record.value.configuration.usersFilter.tenantEnable = isEmpty(record.value?.configuration?.usersFilter?.tenantsIds) ? 'tenantProfile' : 'tenant'
  }
  ...
  updateSchema([
    {
      field: 'configuration.usersFilter.tenantEnable',
      defaultValue: 'tenant',
      ifShow: NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == record.value?.configuration?.usersFilter?.type && userStore.getAuthority == Authority.SYS_ADMIN,
    },
    {
      field: 'configuration.usersFilter.tenantsIds',
      ifShow: NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == record.value?.configuration?.usersFilter?.type && record.value?.configuration?.usersFilter?.tenantEnable == 'tenant',
    },
    {
      field: 'configuration.usersFilter.tenantProfilesIds',
      ifShow: NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == record.value?.configuration?.usersFilter?.type && record.value?.configuration?.usersFilter?.tenantEnable == 'tenantProfile',
    },
    {
      field: 'configuration.usersFilter.customerId',
...
async function handleSubmit() {
  try {
    ...
    if (!isEmpty(data.configuration.usersFilter.tenantProfilesIds) && !isArray(data.configuration.usersFilter.tenantProfilesIds)) {
  ...
  updateSchema([
    {
      field: 'configuration.usersFilter.tenantEnable',
      defaultValue: 'tenant',
      ifShow: NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == value && userStore.getAuthority == Authority.SYS_ADMIN,
    },
    {
      field: 'configuration.usersFilter.tenantsIds',
      ifShow: NotificationRecipientUsersFilter.TENANT_ADMINISTRATORS == value && userStore.getAuthority == Authority.SYS_ADMIN,
