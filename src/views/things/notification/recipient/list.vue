  ...
  <div class="notification-recipient-list">
    <BasicTable @register="registerTable">
      ...
      <template #configurationType="{ record }">
        <Tag color="green">
          {{ NOTIFICATION_RECIPIENT_TYPE_OPTIONS.find(item => item.value ==
            record?.configuration?.type)?.label || record?.configuration?.type }}
        </Tag>
      </template>
      <template #configurationUsersFilter="{ record }">
        <Tag color="purple" v-if="record.configuration.type == NotificationRecipientType.PLATFORM_USERS">
          {{ NOTIFICATION_RECIPIENT_USER_FILTER_OPTIONS.find(item => item.value ==
            record?.configuration?.usersFilter.type)?.label || record?.configuration?.type }}
        </Tag>
      </template>
    </BasicTable>
...
import { notificationTargetList, deleteNotificationTarget } from '/@/api/things/notificationTarget';
import { reactive } from 'vue';
import InputForm from './form.vue';
import { NotificationRecipientType, NOTIFICATION_RECIPIENT_TYPE_OPTIONS, NOTIFICATION_RECIPIENT_USER_FILTER_OPTIONS } from '/@/enums/notificationEnum';
