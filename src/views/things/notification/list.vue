    ...
    <BasicTable @register="registerTable">
      ...
      <template #toolbar>
        <Tooltip placement="top">
          ...
          <CheckCircleOutlined @click="handleMarkAllAsRead" class="table-settings" :style="{ color: 'green' }" />
        </Tooltip>
      </template>
      <template #leftToolbar>
        <Segmented v-model:value="unreadOnly" @change="reload()"
          :options="[{ label: '未读', value: 'true' }, { label: '所有', value: 'false' }]" />
...
import { notificationList, deleteNotification, markAllNotificationAsRead, markNotificationAsRead } from '/@/api/things/notification';
...
function handleDetail(record: Recordable) {
  createConfirm({
    iconType: 'success',
    icon: () => h(Icon, { icon: record.additionalConfig.icon.enabled ? record.additionalConfig.icon.icon : 'ant-design:info-circle-filled', color: record.additionalConfig.icon.enabled ? record.additionalConfig.icon.color : 'blue' },),
    title: () => h('div', { style: { width: '100%' } }, [h('span', { innerHTML: record.subject }), h('span', { style: { float: 'right', 'font-size': '12px', 'font-weight': 'normal', color: 'rgba(0, 0, 0, 0.55)' } }, dayjs(record.createdTime).fromNow())]),
