<template>
  <BasicModal v-bind="$attrs" :showFooter="true" @register="registerModal" width="50%">
    ...
    <BasicForm @register="registerForm" v-show="stepItems.at(currentStep)?.title == '组合'">
      <template #uesTemplate="{ model, field }">
        <div class="w-full flex">
          <Radio.Group v-model:value="model[field]" button-style="solid" style="margin: 1px auto;"
      ...
      <template #deliveryMethods>
        <div class="grid grid-cols-2 gap-2 w-full">
          <div class="border border-neutral-300 rounded-md py-3 px-4">
    ...
    <WEB ref="deliverMethodWebForm" v-if="deliveryMethodEnable.WEB" v-show="stepItems.at(currentStep)?.title == 'WEB'" />
    <SMS ref="deliverMethodSMSForm" v-if="deliveryMethodEnable.SMS" v-show="stepItems.at(currentStep)?.title == 'SMS'" />
    <EMAIL ref="deliverMethodEmailForm" v-if="deliveryMethodEnable.EMAIL"
      v-show="stepItems.at(currentStep)?.title == 'EMAIL'" />
    <SLACK ref="deliverMethodSlackForm" v-if="deliveryMethodEnable.SLACK"
...
<script lang="ts" setup name="NotificationRequestForm">
...
import { NotificationRequest, createNotificationRequest, getNotificationRequestById, getNotificationRequestPreview } from '/@/api/things/notification';
...
const stepItems = computed(() => {
  const items = Object.keys(deliveryMethodEnable.value).filter(key => deliveryMethodEnable.value[key] == true).map(method => { return { title: method } });
...
const inputFormSchemas: FormSchema[] = [
  ...
  { field: 'templateId.entityType', component: 'Input', defaultValue: EntityType.NOTIFICATION_TEMPLATE, ifShow: false },
  {
    label: t('通知模版'),
    field: 'templateId.id',
    component: 'Select',
    ...
    rules: [
      {
        validator(_rule,) {
          return new Promise((resolve, reject) => {
            if (Object.keys(deliveryMethodEnable.value).filter(key => deliveryMethodEnable.value[key] == true).length > 0) {
              return resolve();
            }
            reject(t('至少选择一种接收方式'))
          });
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  if (isEmpty(record.value?.templateId?.id)) {
    ...
    if (record.value?.template?.configuration?.deliveryMethodsTemplates) {
      ...
      (Object.keys(deliveryMethodsTemplates).filter(key => deliveryMethodsTemplates[key].enabled == true) || ['WEB']).forEach(key => {
  ...
  nextTick(() => {
    if (deliverMethodWebForm.value != null) {
      deliverMethodWebForm.value.setFieldsValue(record.value?.template?.configuration?.deliveryMethodsTemplates?.WEB || {});
    }
    if (deliverMethodSMSForm.value != null) {
      deliverMethodSMSForm.value.setFieldsValue(record.value?.template?.configuration?.deliveryMethodsTemplates?.SMS || {});
    }
    if (deliverMethodEmailForm.value != null) {
      deliverMethodEmailForm.value.setFieldsValue(record.value?.template?.configuration?.deliveryMethodsTemplates?.EMAIL || {});
    }
    if (deliverMethodSlackForm.value != null) {
      deliverMethodSlackForm.value.setFieldsValue(record.value?.template?.configuration?.deliveryMethodsTemplates?.SLACK || {});
    }
  })
...
async function validateAndData() {
  try {
    ...
    if (isEmpty(data?.templateId?.id)) {
      ...
      data.template = { name: buildUUID(), notificationType: NotificationType.GENERAL, configuration: { deliveryMethodsTemplates: {} } };
      if (deliverMethodWebForm.value != null) {
...
async function fetchTemplateList() {
  const templateListResult = await notificationTemplateList({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' }, NotificationType.GENERAL);
