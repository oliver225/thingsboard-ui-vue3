  ...
  <BasicModal v-bind="$attrs" :showFooter="true" @register="registerModal" width="50%">
    <template #title>
      ...
      <Steps size="small" v-model:current="currentStep" style="margin-top: 12px">
        <Steps.Step :title="'基础'" />
        <Steps.Step :title="NOTIFICATION_TYPE_OPTIONS.find((item) => item.value == triggerType)?.label || triggerType" />
      </Steps>
    </template>
        ...
        <div class="ant-form-item ">
          <div class="ant-row ant-form-row ">
            <div class="ant-col ant-form-item-label">
              <label for="escalationTable" class="ant-form-item-required ant-form-item-no-colon leading-8">通知顺序</label>
            </div>
            <div class="ant-col ant-form-item-control">
              <InputGroup compact v-for="(escalation, index) in escalationList" :key="index" style="margin-bottom: 8px;">
                <Select v-model:value="escalationList[index].second" :disabled="index == 0"
                  :options="escalationTimeOption" style="width: 25%">
                </Select>
                <Select v-model:value="escalationList[index].targets" mode="multiple" placeholder="请选择通知接收组"
                  style="width: 70%" :options="recipientOptions">
                </Select>
    ...
    <div v-show="currentStep == 1">
      ...
      <ruleEngineLifecycleEvent v-if="NotificationType.RULE_ENGINE_COMPONENT_LIFECYCLE_EVENT == triggerType"
...
<script lang="ts" setup name="NotificationRuleForm">
...
import { getNotificationRuleById, saveNotificationRule, NotificationRule } from '/@/api/things/notificationRule';
import { Authority } from '/@/enums/authorityEnum';
import { NOTIFICATION_TYPE_OPTIONS, NotificationType } from '/@/enums/notificationEnum';
import { notificationTemplateList } from '/@/api/things/notificationTemplate';
import { EntityType } from '/@/enums/entityTypeEnum';
import { notificationTargetList } from '/@/api/things/notificationTarget';
...
const notificationTypeOptions = computed(() => {
  if (userStore.getAuthority == Authority.SYS_ADMIN) {
    return NOTIFICATION_TYPE_OPTIONS.filter(
      item => item.value == NotificationType.ENTITIES_LIMIT
    ...
    return NOTIFICATION_TYPE_OPTIONS.filter(
      item => item.value == NotificationType.ALARM
...
const inputFormSchemas: FormSchema[] = [
  ...
  { field: 'recipientsConfig.triggerType', component: 'Input', defaultValue: notificationTypeOptions.value[0].value, show: false },
  {
    label: t('通知接收组'),
...
async function handleSubmit() {
  try {
    ...
    if (NotificationType.RULE_ENGINE_COMPONENT_LIFECYCLE_EVENT == triggerType.value && ruleEngineLifecycleForm.value) {
...
async function handleTriggerTypeChange(type: string) {
  ...
  const templateListResult = await notificationTemplateList({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' }, type)
  templateOptions.value = templateListResult.data.map(item => {
    return { value: item.id.id, label: item.name, deliveryMethods: Object.keys(item.configuration.deliveryMethodsTemplates as any) }
  })
  const recipientResult = await notificationTargetList({ pageSize: 50, page: 0, sortProperty: 'name', sortOrder: 'ASC' }, type);
  ...
  if (entitiesLimitForm.value) {
    entitiesLimitForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (newPlatformVersionForm.value) {
    newPlatformVersionForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (apiUsageLimitForm.value) {
    apiUsageLimitForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (deviceActivityForm.value) {
    deviceActivityForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (entityActionForm.value) {
    entityActionForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (alarmForm.value) {
    alarmForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (alarmAssignmentForm.value) {
    alarmAssignmentForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (alarmCommentForm.value) {
    alarmCommentForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
  if (ruleEngineLifecycleForm.value) {
    ruleEngineLifecycleForm.value.setFieldsValue({ triggerConfig: record.value?.triggerConfig, additionalConfig: record.value?.additionalConfig });
  }
}
