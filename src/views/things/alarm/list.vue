...
import { getAlarmInfoList, deleteAlarm, getAlarmInfoByEntity } from '/@/api/things/alarm';
...
function wrapFetchParams(fetchParam: any) {
  ...
  const severityList = fetchParam.severityList ? fetchParam.severityList.join(',') : null;
  const statusList = fetchParam.statusList ? fetchParam.statusList.join(',') : null;
  const typeList = ''; //报警类型
  const assigneeId = ''; //委托人
  return { ...fetchParam, page: page, textSearch: searchParam.textSearch, startTime: startTime, endTime: endTime, severityList: severityList, statusList: statusList }
}
