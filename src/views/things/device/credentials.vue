  ...
  <BasicModal v-bind="$attrs" @register="registerModal" @ok="handleSubmit" width="40%"
    ...
    <BasicForm @register="registerForm">
      <template #credentialsId="{ model, field }">
        <Input v-model:value="model[field]" :disabled="userStore.getAuthority != Authority.TENANT_ADMIN"
          placeholder="请输入访问令牌">
        <template #suffix>
          ...
          <Tooltip title="生成" v-if="userStore.getAuthority == Authority.TENANT_ADMIN">
...
<script lang="ts" setup name="DeviceCredentialsModal">
...
import { DeviceInfo, DeviceCredentials, getDeviceCredentialsByDeviceId, updateDeviceCredentials } from '/@/api/things/device';
import { Authority } from '/@/enums/authorityEnum';
import { BasicForm, FormSchema, useForm } from '/@/components/Form';
import { CredentialsType } from '/@/enums/deviceEnum';
import { randomSecret } from '/@/utils/uuid';
...
const inputFormSchemas: FormSchema[] = [
  { field: 'deviceId', component: 'Input', defaultValue: deviceInfo.value?.id, show: false },
  {
    ...
    componentProps: {
      disabled: userStore.getAuthority != Authority.TENANT_ADMIN,
      options: [
