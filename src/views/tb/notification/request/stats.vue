<template>
  <BasicModal v-bind="$attrs" :showFooter="true" :showCancelBtn="false" :canFullscreen="false" @register="registerModal"
    width="40%" @ok="close">
    <template #title>
    ...
    <div class="stats-card-container">
      ...
      <div class="sent" v-if="!isEmpty(stats?.sent)">
        <p class="title">
          ...
          <!-- <UserTransmitIcon /> -->
          <span> {{ Object.keys(stats?.sent).map(key => Number.parseInt(stats?.sent[key])).reduce((a, b) => a + b, 0)
          }}&nbsp;已经发送</span>
        </p>
