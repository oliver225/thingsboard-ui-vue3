      ...
      <Form ref="formRef" :model="formState" layout="vertical">
        ...
        <CollapseContainer title="密码策略" :canExpan="false" class="border border-solid border-neutral-300 mb-4 ">
          <div class="px-4">
            <Row :gutter="24">
              <Col :span="12">
              <Form.Item label="最小密码长度" :name="['passwordPolicy', 'minimumLength']" :rules="[{ required: true }]">
                <InputNumber v-model:value="formState.passwordPolicy.minimumLength" :min="1" style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="最大密码长度" :name="['passwordPolicy', 'maximumLength']" :rules="[{ required: true }]">
                <InputNumber v-model:value="formState.passwordPolicy.maximumLength" :min="1" style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="最少大写字母位数" :name="['passwordPolicy', 'minimumUppercaseLetters']">
                <InputNumber v-model:value="formState.passwordPolicy.minimumUppercaseLetters" :min="0"
                  style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="最少小写字母位数" :name="['passwordPolicy', 'minimumLowercaseLetters']">
                <InputNumber v-model:value="formState.passwordPolicy.minimumLowercaseLetters" :min="0"
                  style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="最少数字位数" :name="['passwordPolicy', 'minimumDigits']">
                <InputNumber v-model:value="formState.passwordPolicy.minimumDigits" :min="0" style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
              <Form.Item label="最少特殊字符位数" :name="['passwordPolicy', 'minimumSpecialCharacters']">
                <InputNumber v-model:value="formState.passwordPolicy.minimumSpecialCharacters" :min="0"
                  style="width: calc(99% )" />
              </Form.Item>
              </Col>
              <Col :span="12">
