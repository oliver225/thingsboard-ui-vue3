<template>
  <div class="debug-setting">
    <Popconfirm
      ...
      :placement="inForm === true ? 'bottom' : 'left'"
      @cancel="handleCancel"
    ...
    >
      ...
      <Tooltip placement="top" :title="label" class="cursor-pointer" v-if="inForm === false">
        <Icon
          :icon="
            !status.failuresEnabled && !status.allEnabled ? 'i-ant-design:bug-outlined' : 'i-ant-design:bug-filled'
          "
          :size="18"
        />
      </Tooltip>
      <Button type="primary" ghost class="debug-button" v-if="inForm === true">
        <Icon icon="i-ant-design:bug-filled" :size="16" />
        <span>{{ label }}</span>
      </Button>
    </Popconfirm>
...
<script setup lang="ts">
  ...
  /** Props 定义 */
  const props = defineProps<{ value: DebugSettings | undefined; inForm: boolean }>();
  /** Emits 定义 */
  const emit = defineEmits<{
    (e: 'update:value', value: DebugSettings): void;
    (e: 'change', value: DebugSettings): void;
  }>();
