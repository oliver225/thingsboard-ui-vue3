    ...
    <template #title>
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-2">
          ...
          <span class="text-base font-medium">{{ connector?.value?.name || 'MQTT Connector' }}</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="h-4 w-px bg-gray-300"></div>
          <span class="text-sm text-gray-500">MQTT Configuration</span>
    ...
    <div class="config">
      <Tabs v-model:activeKey="active" type="line">
        ...
        <template v-if="mode === CONNECTOR_MODE.BASIC">
          <TabPane key="connection">
            <template #tab>
              <div class="flex items-center gap-2">
                <Icon icon="ant-design:link-outlined" class="text-sm" />
                <span>{{ t('tb.gateway.details.connectorConfig.connectionConfig') }}</span>
              </div>
            </template>
            <Connection v-bind="allProps.connection" :ref="(el: any) => setComponentRef('connection', el)" />
          </TabPane>
          <TabPane key="mapping">
            <template #tab>
              <div class="flex items-center gap-2">
          ...
          <TabPane key="requests">
            <template #tab>
              <div class="flex items-center gap-2">
                <Icon icon="ant-design:interaction-outlined" class="text-sm" />
                <span>{{ t('tb.gateway.details.connectorConfig.requestMapping') }}</span>
              </div>
            </template>
            <RequestsMapping v-bind="allProps.requests" :ref="(el: any) => setComponentRef('requests', el)" />
          </TabPane>
          <TabPane key="workers">
            <template #tab>
              <div class="flex items-center gap-2">
                <Icon icon="ant-design:cluster-outlined" class="text-sm" />
                <span>{{ t('tb.gateway.details.connectorConfig.workers') }}</span>
              </div>
            </template>
            <Workers v-bind="allProps.workers" :ref="(el: any) => setComponentRef('workers', el)" />
          </TabPane>
        </template>
...
<script setup lang="ts">
  ...
  import { General, Connection, DataMapping, RequestsMapping, Workers } from './mqtt';
  import { CONNECTOR_MODE, Connectors, CONNECTOR_MODE_OPTIONS } from './types';
  import { MqttGatewayConfig } from './mqtt/type';
  ...
  const configurationJson = ref<MqttGatewayConfig>();
  // 保存初始配置用于重置
  ...
  const allProps = computed(() => ({
    ...
    connection: {
      config: configurationJson.value?.broker,
    },
    workers: {
      config: configurationJson.value?.broker,
    },
    ...
    requests: {
      config: configurationJson.value?.requestsMapping,
    },
  }));
  ...
  async function convertBasicToAdvanced() {
    try {
      ...
      let mergedConfigurationJson: Partial<MqttGatewayConfig> = cloneDeep(
      ...
      for (const [key, comp] of Object.entries(componentRefs.value || {})) {
        ...
        try {
          ...
          if (key === 'general') {
            ...
            // 只更新该组件负责的部分
            if (key === 'connection' || key === 'workers') {
              // connection 和 workers 组件都管理 broker 配置
              if (result.configurationJson.broker) {
                mergedConfigurationJson.broker = {
                  ...(mergedConfigurationJson.broker || {}),
                  ...cloneDeep(result.configurationJson.broker),
                };
              }
            ...
            } else if (key === 'requests' && result.configurationJson.requestsMapping) {
              mergedConfigurationJson.requestsMapping = cloneDeep(result.configurationJson.requestsMapping);
            }
          }
        } catch (error) {
  ...
  async function save() {
    ...
    try {
      ...
      let mergedConfigurationJson: Partial<MqttGatewayConfig> = {
        mapping: [],
        requestsMapping: {
          connectRequests: [],
          disconnectRequests: [],
          ...
          try {
            ...
            if (key === 'general') {
              ...
              if (result.configurationJson.broker) {
                mergedConfigurationJson.broker = {
                  ...(mergedConfigurationJson.broker || {}),
                  ...result.configurationJson.broker,
                };
              }
              if (result.configurationJson.mapping) {
                mergedConfigurationJson.mapping = result.configurationJson.mapping;
              }
              if (result.configurationJson.requestsMapping) {
                mergedConfigurationJson.requestsMapping = result.configurationJson.requestsMapping;
              }
            }
          } catch (error) {
            console.error(`Error getting configuration from ${key}:`, error);
