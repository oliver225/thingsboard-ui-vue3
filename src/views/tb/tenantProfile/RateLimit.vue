  ...
  <div class="tenant-profile-rate-limit">
    ...
    <div>
      <Tooltip :title="'添加限制'" class="mx-4" v-if="edit == true">
        <Icon class="cursor-pointer" :icon="'ant-design:plus-circle-outlined'" :size="24" color="blue"
      ...
      <div class="tenant-profile-rate-limit-modal">
        <div class="edit-container">
          <div v-for="(rate, index) in rateList" :key="index" class="edit-item">
            ...
            <InputNumber v-model:value="rate.second" :min="1" addon-before="每" addon-after="秒"
              :style="{ width: '100%' }" />
            <Tooltip :title="'删除限制'">
              <Icon class="cursor-pointer" :icon="'ant-design:minus-circle-outlined'" :size="24" color="red"
...
function updateRateLimitValue() {
  ...
  emit('update:value', rateList.value.filter(item => item.quantity > 0 && item.second > 0).map(item => `${item.quantity}:${item.second}`).join(','))
}
