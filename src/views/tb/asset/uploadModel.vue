<template>
  ...
  >
    ...
    <div class="asset-upload">
      <div v-if="currentStep === 0" class="asset-upload__panel">
        <Upload.Dragger
      ...
      <div v-else-if="currentStep === 1" class="asset-upload__panel">
        <div class="asset-upload__field" v-show="!isExcelSource">
          <div class="asset-upload__label">CSV 分隔符:</div>
          <Select v-model:value="config.delimiter" :options="delimiterOptions" />
        </div>
        <div class="asset-upload__checks">
          <Checkbox v-model:checked="config.header">第一行包含列名</Checkbox>
          <Checkbox v-model:checked="config.update">更新属性/遥测</Checkbox>
        </div>
      </div>

      <div v-else class="asset-upload__panel">
        <Table
          :bordered="true"
    ...
    <template #footer>
      <div class="asset-upload__footer" v-show="hasPermission(Authority.TENANT_ADMIN)">
        <div>
          <Button v-if="currentStep > 0" @click="handlePrev">
            <Icon :icon="'ant-design:left-outlined'" /> 上一步
          </Button>
        </div>
        <div class="asset-upload__footer-actions">
...
<script lang="ts" setup>
  ...
  const COLUMN_TYPE_OPTIONS = BULK_IMPORT_COLUMN_OPTIONS.filter((item) => {
    return (
      item.value == BulkImportColumnType.NAME ||
      item.value == BulkImportColumnType.TYPE ||
      item.value == BulkImportColumnType.LABEL ||
      item.value == BulkImportColumnType.DESCRIPTION ||
      item.value == BulkImportColumnType.TIME_SERIES ||
      item.value == BulkImportColumnType.SERVER_ATTRIBUTE
    );
  }).map((item) => ({
    label: item.label,
    value: item.value,
  ...
  function requiresKey(type: BulkImportColumnType) {
    return [BulkImportColumnType.SERVER_ATTRIBUTE, BulkImportColumnType.TIME_SERIES].includes(type);
  }
  ...
  function showResult(result: BulkImportResult) {
    createSuccessModal({
      title: '导入完成',
      width: 520,
      content: h('div', { class: 'asset-upload-result' }, [
        h('p', null, `创建资产：${result.created}`),
        h('p', null, `更新资产：${result.updated}`),
        h('p', null, `错误数量：${result.errors}`),
        ...(result.errorsList?.length
          ? [
              h('div', { class: 'asset-upload-result__title' }, '错误详情'),
              h(
                'div',
                { class: 'asset-upload-result__errors' },
                result.errorsList.map((item) => h('div', { class: 'asset-upload-result__error-item' }, item)),
              ),
            ]
          : []),
      ]),
...
<style lang="less">
  ...
  .asset-upload__checks {
    display: flex;
    flex-direction: column;
    gap: 12px;
  }

  .asset-upload__footer {
    display: flex;
  ...
  .asset-upload-result__title {
    margin: 12px 0 8px;
    font-weight: 500;
  }

  .asset-upload-result__errors {
    max-height: 220px;
    overflow: auto;
    padding: 8px 12px;
    background: #fafafa;
  ...
  .asset-upload-result__error-item + .asset-upload-result__error-item {
    margin-top: 8px;
  }
</style>
