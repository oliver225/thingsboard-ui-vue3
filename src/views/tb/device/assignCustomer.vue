<template>
  ...
  >
    ...
    <p class="mb-2">{{ t('tb.device.action.assignCustomerSelectTip') }}</p>
...
<script lang="ts" setup name="ViewsTbDeviceAssignToCustomerForm">
  ...
  async function handleSubmit() {
    if (isEmpty(customerId.value)) {
      showMessage(t('tb.device.action.assignCustomerSelectEmpty'));
      return;
    }
    try {
      setModalProps({ confirmLoading: true });
      const res = await assignDeviceToCustomer(customerId.value, deviceInfo.value.id.id);
      showMessage(t('tb.device.action.assignCustomerSuccess'));
