<template>
  <BasicModal
    ...
    @cancel="handleCancel"
    width="40%"
    :okText="t('tb.ruleChain.node.add')"
  ...
  >
    ...
    <Select
      v-model:value="currentTypes"
      :mode="customRelations ? 'tags' : 'multiple'"
      style="width: 100%"
      :placeholder="t('tb.ruleChain.node.linkLabel')"
      size="large"
    >
...
<script lang="ts" setup name="RuleChainConnectTypeForm">
  ...
  const getTitle = computed(() => ({
    ...
    value:
      currentTypes.value?.length || 0 > 0 ? t('tb.ruleChain.node.editNodeLink') : t('tb.ruleChain.node.addNodeLink'),
  }));
  ...
  async function handleSubmit() {
    if (isEmpty(currentTypes.value)) {
      showMessage(t('tb.ruleChain.node.linkLabelRequired'), 'error');
      return;
    }
