<template>
  <div class="rule-chain-edit">
    ...
    <div class="rule-chain-title">
      {{ record.name }}
      {{ record.root ? t('tb.ruleChain.flow.root') : '' }}
    </div>
    <TeleportContainer />
    ...
    <Space size="middle" class="fixed bottom-8 right-8 z-50">
      <Tooltip :title="t('tb.ruleChain.flow.deleteSelection')">
        <Button
          shape="circle"
          size="large"
          type="primary"
      ...
      <Tooltip :title="t('tb.ruleChain.flow.resetDebug')">
        <Button
          shape="circle"
          size="large"
          type="primary"
      ...
      <Tooltip :title="t('tb.ruleChain.flow.cancelChanges')">
        <Button
          shape="circle"
          size="large"
          style="width: 50px; height: 50px"
      ...
      <Tooltip :title="t('tb.ruleChain.flow.saveChanges')">
        <Button
          shape="circle"
          size="large"
          type="primary"
...
<script lang="ts" setup name="ViewsTbRuleChainFLowIndex">
  ...
  async function fetchData() {
    ...
    if (isEmpty(ruleChainId)) {
      console.log(new Error(t('tb.ruleChain.form.ruleChainEmpty')));
      return;
    }
  ...
  async function renderGraph() {
    ...
    const stencil = new Stencil({
      ...
      placeholder: t('tb.ruleChain.flow.searchRuleNodes'),
      notFoundText: t('tb.ruleChain.flow.notFound'),
    ...
    // 绑定ctrl+v 粘贴
    graph.bindKey('ctrl+v', () => handlePostionPaste());
    // 绑定ctrl+z 撤销
  ...
  async function renderMetaData() {
    ...
    graphRef.value?.addNode({
      ...
      data: {
        descriptor: {
          name: t('tb.ruleChain.flow.input'),
          configurationDescriptor: {
            nodeDefinition: {
              description: t('tb.ruleChain.flow.inputDescription'),
      ...
      if (metaData.value.connections && metaData.value.connections.length > 0) {
        ...
        metaData.value.connections.forEach((connect) => {
          const target = currentConnect.find(
            (item) => item.fromIndex == connect.fromIndex && item.toIndex == connect.toIndex,
          );
          if (target == undefined) {
  ...
  function onNodeChangeData() {
    disableDebugButton.value =
      (graphRef.value?.getNodes().filter((node) => {
        if (isEmpty(node.data?.data?.debugSettings)) {
          return false;
        } else {
          const debugSettings = node.data?.data?.debugSettings as unknown as DebugSettings;
          if (debugSettings.allEnabledUntil && debugSettings.allEnabledUntil > Date.now()) {
            return true;
          } else if (debugSettings.failuresEnabled == true) {
            return true;
          } else {
            return false;
          }
    ...
    if (nodeList?.length) {
      nodeList.forEach((node) => {
        if (!isEmpty(node.data?.data?.debugSettings)) {
          node.setData({
            ...node.data,
            data: { ...node.data.data, debugSettings: null },
          });
        }
      });
    }
  ...
  // 指定位置粘贴
  function handlePostionPaste(x?: number, y?: number) {
  ...
  function onNodeContextMenu({ e, node }) {
    ...
    createContextMenu({
      ...
      items: [
        {
          label: t('tb.ruleChain.flow.details'),
          icon: 'ant-design:align-right-outlined',
          divider: true,
          handler: () => openNodeDetailDrawer(true, { ...node.data }),
        },
        {
          label: t('tb.ruleChain.flow.edit'),
          icon: 'i-clarity:note-edit-line',
          divider: true,
          handler: () => openNodeModal(true, { ...node.data, ruleChainId: record.value.id, nodeId: node.id }),
        },
        {
          label: `${t('common.copyText')} (Ctrl+C)`,
  ...
  function onBlankContextMenu({ e, x, y }) {
    createContextMenu({
      ...
      items: [
        ...
        {
          label: t('tb.ruleChain.flow.cancelChanges'),
          icon: 'ant-design:close-outlined',
          divider: true,
          disabled: disableSaveButton.value,
          handler: handleReduction,
        },
      ],
    });
  }
  ...
  onBeforeRouteLeave((_to, _from, next) => {
    ...
    createConfirm({
      iconType: 'warning',
      title: t('tb.ruleChain.flow.unsavedChanges'),
      content: t('tb.ruleChain.flow.unsavedChangesContent'),
