...
import type { CellFormat, GetColumnsParams } from '../types/table';
...
function handleItem(item: BasicColumn, ellipsis: boolean) {
  ...
  if (dataIndex && isString(dataIndex) && dataIndex.includes('.')) {
...
function handleIndexColumn(
  ...
  const { showIndexColumn, indexColumnProps, isTreeTable, canRowDrag } =
    unref(propsRef);
...
export function useColumns(
  ...
  const getViewColumns = computed(() => {
    ...
    function buildColumns(columns: BasicColumn[]) {
      return columns
        .filter((column) => {
          return hasAccessByCodes([column.auth]) && isIfShow(column);
        })
        .map((column) => {
          if (column.children) {
      ...
      cacheColumns.forEach((item) => {
        const column = {
          ...
          defaultHidden: !columnKeys.includes(
            item.dataIndex_ || (item.key as string),
          ),
        };
  ...
  function updateColumn(data: Partial<BasicColumn> | Partial<BasicColumn>[]) {
    ...
    if (!hasDataIndex) {
      console.error('必须包含 dataIndex 字段。');
      return;
    }
