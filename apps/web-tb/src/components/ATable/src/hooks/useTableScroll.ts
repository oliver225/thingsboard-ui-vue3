...
import type { ComponentRef, Recordable } from '@vben/types';
...
export function useTableScroll(
  ...
  async function calcTableHeight() {
    ...
    if (!isBoolean(pagination)) {
      const footerEl = tableEl.querySelector(
        '.ant-table-footer',
      ) as HTMLElement;
      if (footerEl) {
    ...
    const summaryEl = tableEl.querySelector(
      '.ant-table-summary',
    ) as HTMLElement;
    if (summaryEl) {
    ...
    if (tableData.length === 0) {
      const emptyDataEl = tableEl.querySelector(
