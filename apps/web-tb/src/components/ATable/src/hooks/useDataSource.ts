...
import type { FetchParams, SorterResult } from '../types/table';
...
export function useDataSource(
  ...
  function deleteTableDataRecord(
    ...
    function deleteRecord(dataSource: Recordable[]) {
      ...
      const recordIndex = records
        .map((item) => {
          return typeof rowKeyName === 'function'
            ? dataSource.findIndex((s) => {
                const source = rowKeyName(s, undefined) as string;
                const target = rowKeyName(item, undefined) as string;
                return source === target;
              })
            : dataSource.findIndex((s) => s[rowKeyName] === item[rowKeyName]);
        }) // 取序号
  ...
  async function fetch(opt?: FetchParams): Promise<any> {
    ...
    try {
      ...
      pageParams[pageField] =
        pageParams[pageField] > 0 ? pageParams[pageField] - 1 : 0;
