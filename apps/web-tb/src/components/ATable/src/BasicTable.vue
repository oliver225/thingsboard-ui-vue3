    ...
    <TableHeader v-bind="getHeaderProps" :show-selection-bar="false">
      ...
      <template #toolbar v-if="$slots.tableTitle || $slots.toolbar">
        <a-button
          ...
          @click="handleFormShowToggle()"
          :class="{ active: formShow }"
        >
          <Icon icon="i-ant-design:filter-outlined" />
          {{ formShow ? t('common.hideText') : t('common.queryText') }}
        </a-button>
        ...
        <template #bodyCell="data: any">
          <template v-if="!data.column.customRender">
            <TableAction
              ...
              :actions="
                tableActions.actions && tableActions.actions(data.record)
              "
              :drop-down-actions="
                tableActions.dropDownActions &&
                tableActions.dropDownActions(data.record)
              "
            />
            <slot
