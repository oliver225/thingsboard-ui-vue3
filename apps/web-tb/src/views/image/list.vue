...
const tableAction = {
  actions: (record: Recordable<any>): ActionItem[] => [
    ...
    },
    {
      ...
      onClick: handleEmbed.bind(this, { ...record }),
    },
    {
      label: `${$t('编辑图像')}`,
      tooltip: `${$t('编辑图像')}`,
...
function handleForm(record: any | ResourceInfo) {
  formModalApi
    ...
    .setData({ imageType: record.imageType, resourceKey: record.resourceKey })
    .open();
}
...
function handleDelete(record: any | ResourceInfo) {
  confirm({
    ...
    message.success(`删除图像[${record.title}]成功！`);
    await reload();
  });
}

function handleEmbed(record: any | ResourceInfo) {
  embedModalApi.setData({ ...record }).open();
}
...
<template>
  <Page auto-content-height>
    <Grid
      :table-header-title="$t('图像库')"
      table-header-title-help="这是一个图像库列表"
    >
