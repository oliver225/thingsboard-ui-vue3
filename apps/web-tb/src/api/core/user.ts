...
export function getUserTokenApi(userId: string) {
  return requestClient.get<AuthApi.JwtPair>(`/user/${userId}/token`);
}
...
export function getTenantAdminListApi(params: BasicQuery, tenantId: string) {
  return requestClient.get<Page<UserInfo>>(`/tenant/${tenantId}/users`, {
    params,
  });
}
export function getCustomerUserList(params: BasicQuery, customerId: string) {
  return requestClient.get<Page<UserInfo>>(`/customer/${customerId}/users`, {
    params,
  });
}

export function getUsersForAssignApi(params: BasicQuery, alarmId: string) {
  return requestClient.get<Page<UserInfo>>(`/users/assign/${alarmId}`, {
    params,
  });
}
