...
async function fetch({ page, sort }: any) {
  return getResources({
    ...
    resourceSubType: queryParams.resourceSubType,
  ...
  gridOptions: {
    columns: [
      { title: $t('tb.common.seq'), type: 'seq', width: 60 },
      {
        ...
        slots: { default: 'title' },
        sortable: true,
        title: $t('tb.javascriptLibrary.fields.title'),
      },
      {
        field: 'resourceSubType',
        formatter: ({ cellValue }) => resourceSubTypeLabel(cellValue) || '-',
        title: $t('tb.javascriptLibrary.fields.subType'),
        width: 140,
      },
      {
        field: 'scope',
        formatter: ({ row }) =>
          isSystem(row)
            ? $t('tb.javascriptLibrary.scope.system')
            : $t('tb.javascriptLibrary.scope.tenant'),
        title: $t('tb.javascriptLibrary.fields.scope'),
        width: 100,
      },
      {
        field: 'createdTime',
  ...
  return [
    {
      ...
      tooltip: $t('tb.javascriptLibrary.actions.download'),
    },
    {
      danger: true,
...
function onOpen(row: TbResourceInfo) {
  formModalApi
    .setData({
      readonly: !hasAccessByRoles([Authority.SYS_ADMIN]) && !isSystem(row),
      resourceId: row.id?.id,
    })
    .open();
}
...
async function onDownload(row: TbResourceInfo) {
  ...
  downloadFileFromBlob({
    fileName: row.fileName ?? `${row.title ?? 'module'}.js`,
    source: blob,
  });
}
...
function confirmDelete(row: TbResourceInfo) {
  confirm({
    ...
    content: $t('tb.javascriptLibrary.delete.content'),
    contentMasking: true,
    icon: 'error',
    title: $t('tb.javascriptLibrary.delete.title', { title: row.title }),
  }).catch(() => {});
}
...
function showForceDelete(row: TbResourceInfo, result: TbResourceDeleteResult) {
  confirm({
    ...
    confirmButtonProps: { danger: true, variant: 'destructive' },
    confirmText: $t('tb.javascriptLibrary.forceDelete.button'),
    content: renderResourceReferences(result, {
      intro: $t('tb.javascriptLibrary.forceDelete.content', {
        title: row.title,
      }),
      outro: $t('tb.javascriptLibrary.forceDelete.footer'),
    }),
    contentMasking: true,
    icon: 'error',
    title: $t('tb.javascriptLibrary.forceDelete.title'),
  }).catch(() => {});
}
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('tb.menu.javascriptLibrary')">
      <template #toolbar-actions>
        <div class="flex items-center gap-3">
          <Select
            v-model:value="queryParams.resourceSubType"
            class="w-40"
            allow-clear
            :placeholder="$t('tb.javascriptLibrary.filter.allSubTypes')"
