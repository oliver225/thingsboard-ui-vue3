...
const options = computed(() => {
  ...
  if (showSelectionColumn.value) {
    const columns = Array.isArray(mergedOptions.columns)
      ? mergedOptions.columns
      : [];
    const hasCheckboxColumn = columns.some(
      (column) => (column as { type?: string })?.type === 'checkbox',
    );
...
<template>
  <div :class="cn('flex h-full flex-col rounded-md bg-card', className)">
    <div
      v-show="showTableTitle"
      class="flex items-center gap-1 px-4 py-3 text-[1rem] font-bold"
    >
      <slot name="table-title">
        <span class="text-lg font-semibold">{{ tableTitle }}</span>
      ...
      >
        ...
        <template #toolbar-tools="slotProps">
          ...
          <VxeButton
            ...
            "
            :status="showSelectionColumn ? 'primary' : undefined"
            :title="$t('common.toggleRowSelection')"
            @click="onSelectionColumnToggle"
          />
        </template>
